"""This is the normal test_zipfile test suite but using the
zipfile_aes.AESZipFile object instead of the zipfile.ZipFile object.
"""
import contextlib
import io
import os
...
class AbstractTestsWithSourceFile:
    ...
    def make_test_archive(self, f, compression, compresslevel=None):
        ...
        with zipfile_aes.AESZipFile(f, "w", **kwargs) as zipfp:
    ...
    def test_open_with_pathlike(self):
        ...
        with zipfile_aes.AESZipFile(path, "r", self.compression) as zipfp:
    ...
    def zip_read1_test(self, f, compression):
        ...
        # Read the ZIP archive
        with zipfile_aes.AESZipFile(f, "r") as zipfp, \
    ...
    def test_low_compression(self):
        ...
        # Create the ZIP archive
        with zipfile_aes.AESZipFile(TESTFN2, "w", self.compression) as zipfp:
            zipfp.writestr("strfile", '12')

        # Get an open object for strfile
        with zipfile_aes.AESZipFile(TESTFN2, "r", self.compression) as zipfp:
            with zipfp.open("strfile") as openobj:
                self.assertEqual(openobj.read(1), b'1')
                self.assertEqual(openobj.read(1), b'2')

    def test_writestr_compression(self):
        zipfp = zipfile_aes.AESZipFile(TESTFN2, "w")
    ...
    def test_writestr_compresslevel(self):
        zipfp = zipfile_aes.AESZipFile(TESTFN2, "w", compresslevel=1)
    ...
    def test_read_return_size(self):
        ...
        for test_size in (1, 4095, 4096, 4097, 16384):
            ...
            with zipfile_aes.AESZipFile(io.BytesIO(), "w", self.compression) as zipf:
    ...
    def test_repr(self):
        fname = 'file.name'
        for f in get_files(self):
            with zipfile_aes.AESZipFile(f, 'w', self.compression) as zipfp:
    ...
    def test_per_file_compresslevel(self):
        ...
        compression levels."""
        with zipfile_aes.AESZipFile(TESTFN2, "w", compresslevel=1) as zipfp:
...
class StoredTestsWithSourceFile(AbstractTestsWithSourceFile,
    ...
    def test_absolute_arcnames(self):
        with zipfile_aes.AESZipFile(TESTFN2, "w", zipfile.ZIP_STORED) as zipfp:
    ...
    def test_append_to_zip_file(self):
        ...
        with zipfile_aes.AESZipFile(TESTFN2, "a", zipfile.ZIP_STORED) as zipfp:
            zipfp.writestr("strfile", self.data)
    ...
    def test_append_to_non_zip_file(self):
        ...
        with open(TESTFN2, 'rb') as f:
            ...
            zipfiledata = f.read()
        with io.BytesIO(zipfiledata) as bio, zipfile_aes.AESZipFile(bio) as zipfp:
            self.assertEqual(zipfp.namelist(), [TESTFN])
            self.assertEqual(zipfp.read(TESTFN), self.data)

    def test_read_concatenated_zip_file(self):
        with io.BytesIO() as bio:
            with zipfile_aes.AESZipFile(bio, 'w', zipfile.ZIP_STORED) as zipfp:
                zipfp.write(TESTFN, TESTFN)
        ...
        with zipfile_aes.AESZipFile(TESTFN2) as zipfp:
            self.assertEqual(zipfp.namelist(), [TESTFN])
            self.assertEqual(zipfp.read(TESTFN), self.data)
    ...
    def test_append_to_concatenated_zip_file(self):
        ...
        with zipfile_aes.AESZipFile(TESTFN2, 'a') as zipfp:
            self.assertEqual(zipfp.namelist(), [TESTFN])
            zipfp.writestr('strfile', self.data)
    ...
    def test_ignores_newline_at_end(self):
        ...
        with zipfile_aes.AESZipFile(TESTFN2, "r") as zipfp:
            self.assertIsInstance(zipfp, zipfile_aes.AESZipFile)
    ...
    def test_write_default_name(self):
        ...
        with zipfile_aes.AESZipFile(TESTFN2, "w") as zipfp:
            zipfp.write(TESTFN)
    ...
    def test_write_to_readonly(self):
        ...
        raises a ValueError."""
        with zipfile_aes.AESZipFile(TESTFN2, mode="w") as zipfp:
            zipfp.writestr("somefile.txt", "bogus")

        with zipfile_aes.AESZipFile(TESTFN2, mode="r") as zipfp:
    ...
    def test_add_file_before_1980(self):
        ...
        with zipfile_aes.AESZipFile(TESTFN2, "w", strict_timestamps=False) as zipfp:
            zipfp.write(TESTFN)
...
class AbstractWriterTests:
    ...
    def test_close_after_close(self):
        data = b'content'
        with zipfile_aes.AESZipFile(TESTFN2, "w", self.compression) as zipf:
            w = zipf.open('test', 'w')
            w.write(data)
            w.close()
...
class ExtractTests(unittest.TestCase):
    ...
    def test_sanitize_windows_name(self):
        san = zipfile_aes.AESZipFile._sanitize_windows_name
...
class OtherTests(unittest.TestCase):
    ...
    def test_writestr_extended_local_header_issue1202(self):
        with zipfile_aes.AESZipFile(TESTFN2, 'w') as orig_zip:
            for data in 'abcdefghijklmnop':
                zinfo = zipfile_aes.AESZipInfo(data)
    ...
    def test_close_on_exception(self):
        ...
        try:
            with zipfile_aes.AESZipFile(TESTFN2, "r") as zipfp2:
    ...
    def test_unsupported_version(self):
        ...
        self.assertRaises(NotImplementedError, zipfile_aes.AESZipFile,
                          io.BytesIO(data), 'r')
    ...
    def test_exclusive_create_zip_file(self):
        ...
        with zipfile_aes.AESZipFile(TESTFN2, "x", zipfile.ZIP_STORED) as zipfp:
            zipfp.writestr(filename, content)
        with self.assertRaises(FileExistsError):
            zipfile_aes.AESZipFile(TESTFN2, "x", zipfile.ZIP_STORED)
        with zipfile_aes.AESZipFile(TESTFN2, "r") as zipfp:
    ...
    def test_create_non_existent_file_for_append(self):
        ...
        try:
            with zipfile_aes.AESZipFile(TESTFN, 'a') as zf:
    ...
    def test_close_erroneous_file(self):
        ...
        try:
            zf = zipfile_aes.AESZipFile(TESTFN)
        except zipfile.BadZipFile:
            pass
    ...
    def test_damaged_zipfile(self):
        ...
        for N in range(len(zipfiledata)):
            ...
            self.assertRaises(zipfile.BadZipFile, zipfile_aes.AESZipFile, fp)

    def test_is_zip_valid_file(self):
        """Check that is_zipfile() correctly identifies zip files."""
        # - passing a filename
        with zipfile_aes.AESZipFile(TESTFN, mode="w") as zipf:
            zipf.writestr("foo.txt", b"O, for a Muse of Fire!")
    ...
    def test_non_existent_file_raises_OSError(self):
        ...
        # quickly.
        self.assertRaises(OSError, zipfile_aes.AESZipFile, TESTFN)

    def test_empty_file_raises_BadZipFile(self):
        f = open(TESTFN, 'w')
        f.close()
        self.assertRaises(zipfile.BadZipFile, zipfile_aes.AESZipFile, TESTFN)
    ...
    def test_bad_constructor_mode(self):
        ...
        self.assertRaises(ValueError, zipfile_aes.AESZipFile, TESTFN, "q")
    ...
    def test_bad_open_mode(self):
        ...
        with zipfile_aes.AESZipFile(TESTFN, mode="r") as zipf:
    ...
    def test_bad_compression_mode(self):
        ...
        caught."""
        self.assertRaises(NotImplementedError, zipfile_aes.AESZipFile, TESTFN, "w", -1)
    ...
    def test_unsupported_compression(self):
        ...
        with zipfile_aes.AESZipFile(io.BytesIO(data), 'r') as zipf:
    ...
    def test_comments(self):
        ...
        with zipfile_aes.AESZipFile(TESTFN, mode="r") as zipfr:
        ...
        with zipfile_aes.AESZipFile(TESTFN,mode="w") as zipf:
            ...
            zipf.writestr("foo.txt", "O, for a Muse of Fire!")
        with zipfile_aes.AESZipFile(TESTFN,mode="a") as zipf:
    ...
    def test_unicode_comment(self):
        with zipfile_aes.AESZipFile(TESTFN, "w", zipfile.ZIP_STORED) as zipf:
            zipf.writestr("foo.txt", "O, for a Muse of Fire!")
            with self.assertRaises(TypeError):
                zipf.comment = "this is an error"

    def test_change_comment_in_empty_archive(self):
        with zipfile_aes.AESZipFile(TESTFN, "a", zipfile.ZIP_STORED) as zipf:
    ...
    def test_empty_zipfile(self):
        ...
        zipf = zipfile_aes.AESZipFile(TESTFN, mode="w")
        zipf.close()
        try:
            zipf = zipfile_aes.AESZipFile(TESTFN, mode="r")
        except zipfile.BadZipFile:
            self.fail("Unable to create empty ZIP file in 'w' mode")

        zipf = zipfile_aes.AESZipFile(TESTFN, mode="a")
        zipf.close()
        try:
            zipf = zipfile_aes.AESZipFile(TESTFN, mode="r")
        except:
    ...
    def test_open_empty_file(self):
        ...
        f = open(TESTFN, 'w')
        f.close()
        self.assertRaises(zipfile.BadZipFile, zipfile_aes.AESZipFile, TESTFN, 'r')

    def test_create_zipinfo_before_1980(self):
        self.assertRaises(ValueError,
                          zipfile_aes.AESZipInfo, 'seventies', (1979, 1, 1, 0, 0, 0))
    ...
    def test_zipfile_with_short_extra_field(self):
        ...
        with zipfile_aes.AESZipFile(io.BytesIO(zipdata), 'r') as zipf:
    ...
    def test_open_conflicting_handles(self):
        ...
        with zipfile_aes.AESZipFile(TESTFN2, 'w', zipfile.ZIP_STORED) as zipf:
        ...
        with zipfile_aes.AESZipFile(TESTFN2, 'r') as zipf:
...
class AbstractBadCrcTests:
    def test_testzip_with_bad_crc(self):
        ...
        with zipfile_aes.AESZipFile(io.BytesIO(zipdata), mode="r") as zipf:
            # testzip returns the name of the first corrupt file, or None
...
class DecryptionTests(unittest.TestCase):
    ...
    def setUp(self):
        with open(TESTFN, "wb") as fp:
            fp.write(self.data)
        self.zip = zipfile_aes.AESZipFile(TESTFN, "r")
        with open(TESTFN2, "wb") as fp:
            fp.write(self.data2)
        self.zip2 = zipfile_aes.AESZipFile(TESTFN2, "r")
    ...
    def test_writestr(self):
        for wrapper in (lambda f: f), Tellable, Unseekable:
            with self.subTest(wrapper=wrapper):
                ...
                with zipfile_aes.AESZipFile(wrapper(bf), 'w', zipfile.ZIP_STORED) as zipfp:
                    zipfp.writestr('ones', b'111')
                    zipfp.writestr('twos', b'222')
                self.assertEqual(f.getvalue()[:5], b'abcPK')
                with zipfile_aes.AESZipFile(f, mode='r') as zipf:
    ...
    def test_open_write(self):
        for wrapper in (lambda f: f), Tellable, Unseekable:
            with self.subTest(wrapper=wrapper):
                ...
                f.write(b'abc')
                bf = io.BufferedWriter(f)
                with zipfile_aes.AESZipFile(wrapper(bf), 'w', zipfile.ZIP_STORED) as zipf:
                    with zipf.open('ones', 'w') as zopen:
                        zopen.write(b'111')
                    with zipf.open('twos', 'w') as zopen:
                        zopen.write(b'222')
                self.assertEqual(f.getvalue()[:5], b'abcPK')
                with zipfile_aes.AESZipFile(f) as zipf:
...
class TestsWithMultipleOpens(unittest.TestCase):
    ...
    def make_test_archive(self, f):
        # Create the ZIP archive
        with zipfile_aes.AESZipFile(f, "w", zipfile.ZIP_DEFLATED) as zipfp:
    ...
    def test_read_after_close(self):
        for f in get_files(self):
            ...
            with contextlib.ExitStack() as stack:
                with zipfile_aes.AESZipFile(f, 'r') as zipf:
    ...
    def test_read_after_write(self):
        for f in get_files(self):
            with zipfile_aes.AESZipFile(f, 'w', zipfile.ZIP_DEFLATED) as zipf:
    ...
    def test_many_opens(self):
        ...
        with zipfile_aes.AESZipFile(TESTFN2, mode="r") as zipf:
            for x in range(100):
                zipf.read('ones')
                with zipf.open('ones') as zopen1:
                    pass
    ...
    def test_write_while_reading(self):
        with zipfile_aes.AESZipFile(TESTFN2, 'w', zipfile.ZIP_DEFLATED) as zipf:
            zipf.writestr('ones', self.data1)
        with zipfile_aes.AESZipFile(TESTFN2, 'a', zipfile.ZIP_DEFLATED) as zipf:
            with zipf.open('ones', 'r') as r1:
                ...
                data1 += r1.read()
        self.assertEqual(data1, self.data1)
        with zipfile_aes.AESZipFile(TESTFN2) as zipf:
...
class TestWithDirectory(unittest.TestCase):
    ...
    def test_extract_dir(self):
        with zipfile_aes.AESZipFile(findfile("zipdir.zip")) as zipf:
...
class ZipInfoTests(unittest.TestCase):
    def test_from_file(self):
        zi = zipfile_aes.AESZipInfo.from_file(__file__)
        expected_fname = os.path.basename(__file__)
        self.assertEqual(posixpath.basename(zi.filename), expected_fname)
        self.assertFalse(zi.is_dir())
        self.assertEqual(zi.file_size, os.path.getsize(__file__))

    def test_from_file_pathlike(self):
        zi = zipfile_aes.AESZipInfo.from_file(pathlib.Path(__file__))
        expected_fname = os.path.basename(__file__)
        self.assertEqual(posixpath.basename(zi.filename), expected_fname)
        self.assertFalse(zi.is_dir())
        self.assertEqual(zi.file_size, os.path.getsize(__file__))
    ...
    def test_from_file_bytes(self):
        zi = zipfile_aes.AESZipInfo.from_file(os.fsencode(__file__), 'test')
    ...
    def test_from_file_fileno(self):
        with open(__file__, 'rb') as f:
            zi = zipfile_aes.AESZipInfo.from_file(f.fileno(), 'test')
            self.assertEqual(posixpath.basename(zi.filename), 'test')
            self.assertFalse(zi.is_dir())
            self.assertEqual(zi.file_size, os.path.getsize(__file__))
    ...
    def test_from_dir(self):
        ...
        zi = zipfile_aes.AESZipInfo.from_file(dirpath, 'stdlib_tests')
...
class CommandLineTest(unittest.TestCase):
    ...
    def test_list_command(self):
        zip_name = findfile('zipdir.zip')
        t = io.StringIO()
        with zipfile_aes.AESZipFile(zip_name, 'r') as tf:
            tf.printdir(t)
    ...
    def test_create_command(self):
        ...
        for opt in '-c', '--create':
            try:
                ...
                self.assertEqual(out, b'')
                with zipfile_aes.AESZipFile(TESTFN2) as zf:
    ...
    def test_extract_command(self):
        ...
        for opt in '-e', '--extract':
            with temp_dir() as extdir:
                ...
                self.assertEqual(out, b'')
                with zipfile_aes.AESZipFile(zip_name) as zf:
