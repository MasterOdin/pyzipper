...
class StoredTestZip64InSmallFiles(AbstractTestZip64InSmallFiles,
    ...
    def make_zip64_file(
        ...
        disk_num_64_set=False, disk_num_extra=False,
    ):
        ...
        actual_disk_num = 0
        local_zip64_q_fields = []
        local_zip64_i_fields = []
        central_zip64_q_fields = []
        central_zip64_i_fields = []

        file_size = actual_size
        if file_size_64_set:
            file_size = 0xffffffff
            if file_size_extra:
                local_zip64_q_fields.append(actual_size)
                central_zip64_q_fields.append(actual_size)
        ...
        if header_offset_64_set:
            ...
            if header_offset_extra:
                central_zip64_q_fields.append(actual_header_offset)
        header_offset = struct.pack("<L", header_offset)

        disk_num = actual_disk_num
        if disk_num_64_set:
            disk_num = 0xffff
            if disk_num_extra:
                central_zip64_i_fields.append(actual_disk_num)
        disk_num = struct.pack("<H", disk_num)

        local_zip_64_fields = local_zip64_q_fields + local_zip64_i_fields
        local_zip_64_field_size = 8 * len(local_zip64_q_fields) + 4 * len(local_zip64_i_fields)
        local_extra = struct.pack(
            '<HH' + 'Q'*len(local_zip64_q_fields) + 'I'*len(local_zip64_i_fields),
            0x0001,
            local_zip_64_field_size,
            *local_zip_64_fields
        )

        central_zip_64_fields = central_zip64_q_fields + central_zip64_i_fields
        central_zip_64_field_size = (
            8 * len(central_zip64_q_fields) + 4 * len(central_zip64_i_fields)
        )
        central_extra = struct.pack(
            '<HH' + 'Q'*len(central_zip64_q_fields) + 'I'*len(central_zip64_i_fields),
            0x0001,
            central_zip_64_field_size,
            *central_zip_64_fields
        )

        central_dir_size = struct.pack('<Q', 58 + central_zip_64_field_size)
        offset_to_central_dir = struct.pack('<Q', 50 + local_zip_64_field_size)

        local_extra_length = struct.pack("<H", 4 + local_zip_64_field_size)
        central_extra_length = struct.pack("<H", 4 + central_zip_64_field_size)
    ...
    def test_generated_valid_zip64_extra(self):
        ...
        # A file without any zip64 fields
        with zipfile.ZipFile(io.BytesIO(self.make_zip64_file())) as zf:
            zinfo = zf.infolist()[0]
        ...
        params = (
            ...
            {"disk_num_64_set": True, "disk_num_extra": True},
        )
